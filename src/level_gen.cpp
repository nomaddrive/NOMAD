...
static inline bool isAgentControllable(Engine &ctx, Entity agent, bool markAsExpert = false) {
    ...
    // Check if we've reached the maximum number of controlled agents
    if (ctx.data().numControlledAgents >= ctx.data().params.maxNumControlledAgents) {
        return false;
    }
    ...
    // Check if agent has valid trajectory at first timestep
    if (!ctx.get<Trajectory>(agent_iface).valids[0]) {
        return false;
    }
    
    // Check if this is a static agent and if static agents are allowed
    bool isStatic = ctx.get<ResponseType>(agent) == ResponseType::Static;
    if (isStatic && !ctx.data().params.allowStaticAgentControl) {
        return false;
    }
    
    // Check if this is marked as expert and if expert agents are allowed
    if (markAsExpert && !ctx.data().params.allowExpertAgentControl) {
        return false;
    }
    
    // For dynamic agents or when static agents are allowed, agent is controllable
    return true;
}
