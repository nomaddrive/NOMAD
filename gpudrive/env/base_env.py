...
class GPUDriveGymEnv(gym.Env, metaclass=abc.ABCMeta):
    ...
    def _setup_environment_parameters(self):
        ...
        params.mirrorX = self.config.mirror_x
        
        # Set controllable agent selection parameters
        self._set_controllable_agent_params(params)
    ...
    def _set_controllable_agent_params(self, params):
        """Sets the parameters that control which agents are controllable.

        Args:
            params (object): Parameters object to update based on controllable agent selection.

        Returns:
            object: Updated parameters with controllable agent selection settings.
        """
        selection = self.config.controllable_agent_selection
        
        if selection == "all_agents":
            # All added agents are controlled (including static and marked_as_expert)
            params.allowStaticAgentControl = True
            params.allowExpertAgentControl = True
        elif selection == "no_static":
            # All added agents except static (including marked_as_expert)
            params.allowStaticAgentControl = False
            params.allowExpertAgentControl = True
        elif selection == "no_expert":
            # All added agents except marked_as_expert (including static)
            params.allowStaticAgentControl = True
            params.allowExpertAgentControl = False
        elif selection == "no_static_no_expert":
            # All agents except static and marked_as_expert
            params.allowStaticAgentControl = False
            params.allowExpertAgentControl = False
        else:
            raise ValueError(f"Invalid controllable agent selection: {selection}")
