...
class PufferGPUDrive(PufferEnv):
    ...
    def __init__(
        ...
        action_space_dx_disc=256,
        action_space_dy_disc=256,
        action_space_dyaw_disc=256,
        ego_state=True,
        road_map_obs=True,
        partner_obs=True,
        norm_obs=True,
        ...
        **kwargs,
        ...
        env_config = EnvConfig(
            ...
            dx=torch.round(torch.linspace(-2.0, 2.0, action_space_dx_disc), decimals=3),
            dy=torch.round(torch.linspace(-2.0, 2.0, action_space_dy_disc), decimals=3),
            dyaw=torch.round(torch.linspace(-torch.pi/4, torch.pi/4, action_space_dyaw_disc), decimals=3),
            use_vbd=use_vbd,
        ...
        if isinstance(self.single_action_space, gymnasium.spaces.Discrete):
            self.actions = torch.zeros(
                (self.num_worlds, self.max_cont_agents_per_env), dtype=torch.int64
            ).to(self.device)
        elif isinstance(self.single_action_space, gymnasium.spaces.MultiDiscrete):
            self.actions = torch.zeros(
                (self.num_worlds, self.max_cont_agents_per_env, len(self.single_action_space.nvec)), dtype=torch.int64
            ).to(self.device)
        else:
    ...
    def step(self, action):
        ...
        if len(done_worlds) > 0:
            ...
            num_off_road = torch.where(self.offroad_in_episode[done_worlds, :][controlled_mask]> 0, 
                                       1, 0).sum()
            off_road_rate = num_off_road / (num_finished_agents + 1e-6)

            num_collision = torch.where(self.collided_in_episode[done_worlds, :][controlled_mask]> 0, 
                                       1, 0).sum()
            collision_rate = num_collision / (num_finished_agents + 1e-6)

            num_goal_achieved = self.env.get_infos().goal_achieved[done_worlds, :][controlled_mask].sum()
            goal_achieved_rate = num_goal_achieved / (num_finished_agents + 1e-6)
                ...
                info_lst.append(
                    {
                        ...
                        "num_off_road": num_off_road.item(),
                        "num_collisions": num_collision.item(),
                        "num_goal_achieved": num_goal_achieved.item(),
