...
class EnvConfig:
    ...
    accel_actions: torch.Tensor = torch.round(
        torch.linspace(-6.0, 6.0, 2000), decimals=3
    )
    head_tilt_actions: torch.Tensor = torch.Tensor([0])

    # Delta Local dynamics model
    dx: torch.Tensor = torch.round(torch.linspace(-2.0, 2.0, 256), decimals=3)
    dy: torch.Tensor = torch.round(torch.linspace(-2.0, 2.0, 256), decimals=3)
    dyaw: torch.Tensor = torch.round(
        torch.linspace(-3.14/4, 3.14/4, 256), decimals=3
    )
    ...
    # Controllable agent selection mode
    # Options:
    # - "all_agents": All added agents are controlled (including static and marked_as_expert)
    # - "no_static": All added agents except static (including marked_as_expert) 
    # - "no_expert": All added agents except marked_as_expert (including static)
    # - "no_static_no_expert": All agents except static and marked_as_expert (default)
    controllable_agent_selection: str = "no_static_no_expert"

    mirror_x: bool = False  # Whether to mirror the data along the x-axis
...
class ControllableAgentSelection(Enum):
    """Enum for selecting which agents are controllable."""

    ALL_AGENTS = "all_agents"  # All added agents are controlled (including static and marked_as_expert)
    NO_STATIC = "no_static"    # All added agents except static (including marked_as_expert)
    NO_EXPERT = "no_expert"    # All added agents except marked_as_expert (including static)
    NO_STATIC_NO_EXPERT = "no_static_no_expert"  # All agents except static and marked_as_expert
