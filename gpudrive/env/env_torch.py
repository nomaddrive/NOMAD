...
class GPUDriveTorchEnv(GPUDriveGymEnv):
    ...
    def _apply_actions(self, actions):
        ...
        if (
            ...
            # Multi-discrete (3 indices) lookup case: action_keys_tensor shape (n1, n2, n3, 3)
            if (
                hasattr(self, "action_keys_tensor")
                and self.action_keys_tensor is not None
                and self.action_keys_tensor.dim() == 4
                and actions.dim() == 3
                and actions.shape[-1] == 3
                and actions.dtype in (torch.int32, torch.int64)
            ):
                idx0, idx1, idx2 = actions.unbind(-1)
                action_value_tensor = self.action_keys_tensor[idx0, idx1, idx2]

            elif actions.dim() == 2:  # (num_worlds, max_agent_count)
                # Single-discrete index mapping (composite index)
                actions = torch.nan_to_num(actions, nan=0).long().to(self.device)
                action_value_tensor = self.action_keys_tensor[actions]

            elif actions.dim() == 3:
                if actions.shape[2] == 1:  # single-discrete with extra dim
                    actions = actions.squeeze(dim=2).to(self.device)
                    action_value_tensor = self.action_keys_tensor[actions]
                elif actions.dtype in (torch.int32, torch.int64) and actions.shape[-1] == 3 and hasattr(self, "action_keys_tensor") and self.action_keys_tensor.dim() == 4:
                    # Another form of multi-discrete indices
                    idx0, idx1, idx2 = actions.unbind(-1)
                    action_value_tensor = self.action_keys_tensor[idx0, idx1, idx2]
                else:
                    # Already real-valued continuous (or provided directly)
    ...
    def _set_multi_discrete_action_space(self) -> None:
        """Configure the multi-discrete action space based on dynamics model."""
        if self.config.dynamics_model == "delta_local":
            self.dx = self.config.dx.to(self.device)
            self.dy = self.config.dy.to(self.device)
            self.dyaw = self.config.dyaw.to(self.device)
            ...
            raise ValueError(
                f"Multi-discrete action space is currently not supported for dynamics_model: {self.config.dynamics_model}."
            )
        # Build full cartesian grid lookup tensor: shape (n1, n2, n3, 3)
        if self.config.dynamics_model == "delta_local":
            dim_tensors = [self.dx, self.dy, self.dyaw]
        else:  # classic / bicycle
            dim_tensors = [self.accel_actions, self.steer_actions, self.head_actions]

        n1, n2, n3 = [len(t) for t in dim_tensors]
        g0 = dim_tensors[0].view(n1, 1, 1).expand(n1, n2, n3)
        g1 = dim_tensors[1].view(1, n2, 1).expand(n1, n2, n3)
        g2 = dim_tensors[2].view(1, 1, n3).expand(n1, n2, n3)
        self.action_keys_tensor = torch.stack([g0, g1, g2], dim=-1)  # (n1, n2, n3, 3)

        # MultiDiscrete action space expects per-dimension index output
        nvec = [n1, n2, n3]
