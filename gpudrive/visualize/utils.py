...
def plot_object_ids(
    ...
    """Plots object IDs at specified (x, y) coordinates."""
    # Draw agent id above the bounding box
    for x, y, id in zip(bboxes[:, 0], bboxes[:, 1], bboxes[:, 5]):
        ax.text(x, y, int(id), color=color, zorder=6)