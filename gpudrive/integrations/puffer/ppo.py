...
class ExpertDataset:
    def __init__(self, path, device="cpu"):
        self.device = device
        data = np.load(path)
        self.obs = torch.tensor(data['obs'], device=device)
        self.actions = torch.tensor(data['actions'], device=device)
        self.data_size = len(self.obs)
        print(f"Expert Dataset loaded with {self.data_size} entries.")

    def sample(self, batch_size):
        indices = torch.randint(0, self.data_size, (batch_size,), device=self.device)
...
def create(config, vecenv, policy, optimizer=None, wandb=None):
    ...
    expert_dataset = None
    if hasattr(config, "expert_data_path") and config.expert_data_path:
         expert_dataset = ExpertDataset(config.expert_data_path, device=config.device)

    if config.kl and expert_dataset is None:
        # Deepcopy the uncompiled policy to avoid copying the torch.compile wrapper
        ref_policy = copy.deepcopy(uncompiled_policy)

        ref_policy.to(config.device)
        ref_policy.eval()
    ...
    for epoch in range(config.update_epochs):
        ...
        for mb in range(experience.num_minibatches):
            ...
            with profile.train_misc:
                ...
                # different implementation of KL regularization: https://github.com/Emerge-Lab/nocturne_lab/blob/412b04677b1be2beec98a577b375d8dfe1809e14/algorithms/ppo/sb3/reg_ppo.py#L105
                if config.kl:
                     # Behavior Cloning Loss (Imitation Is Not Enough)
                     if data.expert_dataset is not None:
                        expert_obs, expert_actions = data.expert_dataset.sample(obs.shape[0])
                        
                        # We need logits for the expert observations.
                        # Assuming data.policy has a get_logits method or similar (based on baselines/bc/bc.py)
                        if hasattr(data.policy, "get_logits"):
                             logits = data.policy.get_logits(expert_obs)
                             if isinstance(logits, list): # MultiDiscrete
                                bc_loss = 0.
                                for i, l in enumerate(logits):
                                    bc_loss += torch.nn.functional.cross_entropy(l, expert_actions[:, i])
                             else:
                                bc_loss = torch.nn.functional.cross_entropy(logits, expert_actions)
                        else:
                             # Fallback: if get_logits is not available, we can't easily compute cross entropy on raw actions
                             # unless we reimplement the policy forward pass logic here.
                             # For safety, let's assume get_logits exists as it was in the user provided baseline.
                             bc_loss = torch.tensor(0., device=config.device)

                        kl_loss = bc_loss # Reuse kl_loss variable to pass to total loss
                     
                     # Standard KL Regularization (Reference Policy)
                     elif data.ref_policy is not None:
                        assert lstm_state is None
                        obs_view = obs.reshape(
                            -1, *data.vecenv.single_observation_space.shape
                        )
                        # Get distributions list for each action head
                        newdistribution = data.policy.get_distribution(
                            obs_view
                        )
                        with torch.no_grad():
                            refdistribution = data.ref_policy.get_distribution(
                                obs_view
                            )
                        kl_loss = 0.
                        for new_dist, ref_dist in zip(newdistribution, refdistribution):
                            # Add KL regularization (both policy and value)
                        ...
                        kl_loss /= len(newdistribution)
                        if config.device == "cuda":
                            torch.cuda.synchronize()
                     else:
                        kl_loss = torch.tensor(0., device=config.device)
                else:
                    kl_loss = torch.tensor(0., device=config.device)
            ...
            with profile.train_misc:
                ...
                losses.kl_loss = kl_loss.item() / num_update_iters
...
def save_checkpoint(data, save_checkpoint_to_wandb=False):
    ...
    # Save training state
    # Support single-discrete (legacy) and multi-discrete policies
    policy_obj = data.uncompiled_policy
    action_dim_value = getattr(policy_obj, "action_dim", None)
    action_dims_value = getattr(policy_obj, "action_dims", None)

    state = {
        "parameters": policy_obj.state_dict(),
        "optimizer_state_dict": data.optimizer.state_dict(),
        "global_step": data.global_step,
        "agent_step": data.global_step,
        "update": data.epoch,
        "model_name": model_name,
        "model_class": policy_obj.__class__.__name__,
