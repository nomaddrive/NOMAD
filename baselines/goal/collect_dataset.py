...
def generate_goal_dataset(
    ...
    for i, batch in tqdm(enumerate(data_loader), desc="Loading data batches"):
        ...
        agent_mask_list.append(valid_mask.cpu())

        cont_agent_mask = env.cont_agent_mask  # (num_worlds, max_agents)
        cont_agent_mask_list.append(cont_agent_mask.cpu())
        ...
        # find mask
        roadgraph_mask = global_roadgraph.segment_height > 0.  # (num_worlds, num_road_points)
        ...
        roadgraph_mask_list.append(roadgraph_mask.cpu())

    agent_states_all = torch.cat(agent_state_list, dim=0)
    agent_mask_all = torch.cat(agent_mask_list, dim=0)
    cont_agent_mask_all = torch.cat(cont_agent_mask_list, dim=0)
    goal_positions_all = torch.cat(goal_position_list, dim=0)
    roadgraph_all = torch.cat(roadgraph_list, dim=0)
    roadgraph_mask_all = torch.cat(roadgraph_mask_list, dim=0)
    ...
    torch.save({
        ...
        "agent_mask": agent_mask_all,         # (num_worlds, max_agents)
        "cont_agent_mask": cont_agent_mask_all,  # (num_worlds, max_agents)
        "goal_positions": goal_positions_all,   # (num_worlds, max_agents, 2)
        "roadgraph": roadgraph_all,           # (num_worlds, num_road_points, 27)
        "roadgraph_mask": roadgraph_mask_all,   # (num_worlds, num_road_points)
...
if __name__ == "__main__":
    ...
    parse.add_argument("--config", "-c", default="baselines/goal/config/goal_data_generation.yaml", type=str, help="Path to the configuration file.")
