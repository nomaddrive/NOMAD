...
def evaluate(
    ...
    expert_policy_mode:
        ...
        "random" -> use random policy.
        "constant_velocity" -> use constant velocity policy.
    """
    ...
    if expert_policy_mode == "random":
        print("Collecting random trajectories...")
        simulated_trajectories = evaluator.collect_simulated_random_trajectories(env, policy)
    elif expert_policy_mode == "constant_velocity":
        print("Collecting constant velocity trajectories...")
        simulated_trajectories = evaluator.collect_simulated_constant_velocity_trajectories(env)
    else:
...
if __name__ == "__main__":
    ...
    parse.add_argument("--config", "-c", default="baselines/eval/config/wosac_evaluate.yaml", type=str, help="Path to the configuration file.")
    parse.add_argument("--model_cpt_dir", help="Path to the model checkpoint directory.", type=str)
    parse.add_argument("--expert_policy_mode", default="none", choices=["none", "multi_discrete", "continuous", "random", "constant_velocity"], help="Expert policy mode.", type=str)
    ...
    if args.expert_policy_mode in ["multi_discrete", "none", "random", "constant_velocity"]:
        config.train.action_type = "multi_discrete"
    elif args.expert_policy_mode == "continuous":
        config.train.action_type = "continuous"
    ...
    if args.expert_policy_mode == "none":
        ...
        model_files = [f for f in os.listdir(model_cpt_dir) if f.endswith(".pt")][::config.train.eval_freq]
        if not model_files:
        ...
        for file in model_files:
            ...
            result_json_path = results_dir / f"{ckpt_name}.wosac_stats.json"

            if result_json_path.exists():
                print(f"Skipping evaluation for {file}, results found at {result_json_path}")
                try:
                    ...
                    print(f"Failed to load existing results for {file}: {e}")
                continue
        ...
        wosac_result = evaluate(
            ...
            data_loader, 
            policy=None, 
            expert_policy_mode=args.expert_policy_mode
        )
        # Save per-checkpoint JSON
        with open(results_dir / f"expert_policy_mode_{args.expert_policy_mode}_wosac_stats.json", "w") as f:
            json.dump({"results": wosac_result}, f, indent=2)
        print(f"Saved results to {results_dir / f'expert_policy_mode_{args.expert_policy_mode}_wosac_stats.json'}")
